...
DC_HOST = "app.workato.com" if DC == "us" else f"app.{DC}.workato.com"
...
# ---------------- HTTP ----------------
# Synthetic stubs used in DRY_RUN so no network calls are made at all.
# Keys are path prefixes; values are stub response bodies (JSON strings).
# These cover every endpoint that the script reads back (GETs for setup + any
# POST response whose body is parsed for an id).
_DRY_STUBS = {
    "GET /api/agentic/genies":      json.dumps({"data": []}),
    "GET /api/users/me":            json.dumps({"email": "dry-run@example.com", "root_folder_id": 1}),
    "GET /api/agentic/skills":      json.dumps({"data": []}),
    "GET /api/iam/user_groups":     json.dumps({"data": []}),
    "GET /api/iam/users":           json.dumps({"data": [{"id": "dry-idp-1", "email": "dry-run@example.com"}]}),
    "POST /api/folders":            json.dumps({"id": "dry-fld-1", "project_id": "dry-proj-1"}),
    "POST /api/agentic/genies":     json.dumps({"data": {"id": "dry-gin-1"}}),
    "POST /api/recipes":            json.dumps({"id": "dry-rcp-1"}),
    "POST /api/iam/user_groups":    json.dumps({"data": {"id": "dry-ug-1"}}),
    "POST /api/agentic/genies/clients": json.dumps({"data": {"client_id": "dry-cid-1", "api_key": "dry-key-1"}}),
}
...
def call(method, path, body=None, base=DEV_BASE):
    ...
    if DRY_RUN:
        # Match longest prefix first so specific paths beat more general ones.
        candidates = [k for k in _DRY_STUBS if path.startswith(k.split(" ", 1)[1]) and k.startswith(method)]
        stub_key = max(candidates, key=lambda k: len(k.split(" ", 1)[1])) if candidates else None
        if stub_key is not None:
            print(f"    [DRY_RUN] {method} {url} (stubbed)")
            return 200, _DRY_STUBS[stub_key]
        # Any other call (PUT, DELETE, un-mapped POST): print and return empty ok.
